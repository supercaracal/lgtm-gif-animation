...
#define GIF_LGTM_DATA_COLORS_SIZE 755
#define GIF_LGTM_DATA_BLOCKS_SIZE 749
#define GIF_LGTM_DATA_SIZE (755 + 749 + 8 + 13)
...
append_lgtm_bytes(struct gif_bytes *bytesp)
{
  ...
  for (i = 0; i < GIF_LGTM_DATA_BLOCKS_SIZE; ++i) {
    bytesp->buf[bytesp->idx++] = lgtm_blocks[i];        // Block Data
  }
